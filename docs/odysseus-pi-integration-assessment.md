...
> **Phase 2 end-to-end — 2026-06-09: ✅ PASS (cross-session memory works).** Built and validated against real `gemma4:e4b`/Ollama: a fact recorded by the agent in session A (`manage_memory` add → Odysseus memory) is recalled in a **fresh** session B purely via the bridge's `before_agent_start` injection (no tool call, no shared pi session) — the essence of Gain #1. Two integration bugs were found and fixed during validation: (1) the model provider must be defined in a per-session `models.json` (read via `$PI_CODING_AGENT_DIR`), not via the extension's `registerProvider` (pi resolves `--provider` before extension factories load → "Unknown provider" in rpc mode); (2) the subprocess must NOT use `DETACHED_PROCESS` (it breaks stdio pipes to the `pi.cmd` shim, so the prompt never reaches pi). Commits: baseline → Phase 1 → Phase 2 → hardening → fix.
>
